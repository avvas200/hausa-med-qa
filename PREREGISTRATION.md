...
**Deviation 4: E1 sample specification (decided after the H1–H3 results, before any E1 item was drawn or coded).**
- *What was specified:* section 3 says E1 codes "50 discordant items" but does not say from which model. The sample is 25 items from Llama-3.2-3B-Instruct and 25 from MedGemma-4B-IT, where discordant means correct in `en_p0` and wrong in `ha_p0` under the primary (parsed) scoring. Items are drawn with a fixed seed (`E1_SEED` in `src/config.py`); an item may be drawn for both models, and each (model, item) row is coded separately. The four pre-registered categories are used, checked in a fixed order (option-format failure, medical-term loss, translation error, reasoning failure; the first that applies is used). The coding sheet hides which model produced each row, and rows are shuffled.
- *Why these models:* Llama-3.2-3B is the H4 intervention model. MedGemma-4B is the only model whose Hausa accuracy is clearly above guessing, so its errors are the most informative about translation. Qwen2.5-3B is left out because about half of its Hausa outputs could not be parsed, and on parsed items its accuracy is at guessing level.
- *Status:* E1 remains exploratory, with no hypothesis test.